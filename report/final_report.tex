...
\author{
\begin{tabular}{c}
Jinting Liu \quad Student ID: 3036579758 \\
Junan Yao \quad Student ID: 3036514203 \\
Zhenyu Yang \quad Student ID: 3036514095 \\
...
More specifically, the GARCH-Bollinger strategy is reported as a unit-position ratio-return strategy. It does not allocate HKD capital, apply futures multiplier scaling, or use dynamic position sizing. In contrast, the LGBM and GRPO risk-managed results are HKD equity-curve results with futures multiplier and position sizing. Therefore, the GARCH-Bollinger return should be interpreted as signal-level profitability, while LGBM and GRPO returns are portfolio-level results. The three strategies are comparable in terms of signal direction, drawdown behavior, and benchmark outperformance, but their return magnitudes are not directly one-to-one comparable.
...
Table~\ref{tab:lgbm-test} reports the naked, unconstrained LGBM backtest as a diagnostic result. These results are very large, with raw test returns above 1000\% in the original-test scenario and bar-level Sharpe ratios above 20 in some cases. However, these figures should not be interpreted as directly implementable performance. Minute-level Sharpe ratios use a very large annualization factor, and high-frequency backtests are sensitive to small execution assumptions. When a comprehensive risk management framework --- incorporating position sizing, volatility-adjusted exposure, daily drawdown limits, and consecutive-loss halts --- was applied, performance became substantially more realistic, delivering a total return of approximately 40.1622\%, a maximum drawdown of only -1.45\%, and a win rate around 66\%. The risk-managed curve is therefore the more realistic LGBM result used in the BAH benchmark comparison.

The poor performance of the night-specific LGBM model is also informative. Although it is trained on night-session data, it performs worse than the original model on the night test set. One possible explanation is that night-session bars contain more execution noise, thinner liquidity, wider effective spreads, and stronger overseas-news shocks. A model trained only on night data may therefore overfit noisy short-term patterns that do not persist out of sample. The original model, trained on the broader full-session dataset, appears to learn more stable features and generalizes better to night trading.
...
To provide a direct passive benchmark, Figure~\ref{fig:bah-strategy-comparison} compares the cumulative test-period performance of the three strategy lines with buy-and-hold (BAH) benchmarks. All curves are normalized to start from 1.0, so the figure focuses on relative cumulative performance rather than initial capital. The three panels use the test period corresponding to each strategy output, rather than a single identical calendar window. The GARCH-Bollinger panel uses the original test period from 2019-10-25 to 2020-06-09. The LGBM panel uses the validation-selected risk-managed equity curve over the night-test period from 2019-10-09 to 2020-03-16. The GRPO panel uses the dynamic-risk day-test equity curve from 2019-11-20 to 2020-06-09.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{bah_strategy_cumulative_curves_lgbm_validbest.png}
    \caption{Buy-and-hold benchmark comparison for the three strategy lines on their corresponding test periods. All curves are normalized to start from 1.0. The panels are benchmark checks within each strategy, not a direct ranking across identical test windows.}
...
\begin{enumerate}
    ...
    \item Shao, Z., Wang, P., Zhu, Q., Xu, R., Song, J., Bi, X., Zhang, H., Zhang, M., Li, Y. K., Wu, Y., and Guo, D. (2024). DeepSeekMath: Pushing the limits of mathematical reasoning in open language models. \textit{arXiv preprint arXiv:2402.03300}.
